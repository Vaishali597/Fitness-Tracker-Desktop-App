...
class FitTrack(QWidget):
    ...
    def delete_workout(self):
        ...
        confirm = QMessageBox.question(self,"Are you sure?","Delete this workout",QMessageBox.Yes |
        QMessageBox.No)
    ...
    def calculate_calories(self):
        ...
        try:
            ...
            normalized = [(c - min_cal) / (max_cal - min_cal) if max_cal != min_cal else 0 for c in calories]

            self.figure.clear()
            ax = self.figure.add_subplot(111)

            ax.scatter(distances, calories, c=normalized, cmap="viridis")
            ax.set_title("Distance vs Calories")
            ax.set_xlabel("Distance")
            ax.set_ylabel("Calories")
            cbar = ax.figure.colorbar(ax.collections[0], label="Normalized Ca;ories")
    ...
    #         distance.append(float(distance))
    #         calorie.append(float(calorie))

    #     try:
    #         min_calorie = min(calorie)
    #         max_calorie = max(calorie)
    #         normalized_calories = [(calorie - min_calorie) / (max_calorie-min_calorie) for calorie in calorie]

    #         plt.style.use("Solarize_Light2")
    #         ax = self.figure.subplots()
    #         ax.scatter(distance, calorie, c= normalized_calories, cmap="viridis", label="Data Points")
    #         ax.set_title("Distance Vs. Calories")
    #         ax.set_xlabel("Distance")
    #         ax.set_ylabel("Calories")
    #         cbar = ax.figure.colorbar(ax.collections[0], label="Normalized Ca;ories")
